...
export function AnimalsToolbar({ search, status, sortDirection, onSearchChange, onStatusChange, onSortDirectionChange }: AnimalsToolbarProps) {
    ...
    return (
        <div className="rounded-[28px] border border-orange-100/70 bg-white/82 p-5 shadow-[0_18px_50px_-34px_rgb(15_23_42/0.28)] backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-950/60 md:p-6">
            <div className="mb-4 border-b border-slate-200/70 pb-4 dark:border-slate-800">
                ...
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">Refine a lista por codigo, nome, especie, status ou ordem de exibicao.</p>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_260px_260px]">
                ...
                <label className="flex flex-col gap-1.5">
                    ...
                    <Select
                        value={["Adotado", "Disponivel", "Em processo", "Todos"].map((v) => ({ value: v as AnimalStatus | "Todos", label: v })).find((o) => o.value === status) ?? null}
