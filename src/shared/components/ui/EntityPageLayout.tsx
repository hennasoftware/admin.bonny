...
export function EntityPageHeader({ eyebrow, title, description, action }: EntityPageHeaderProps) {
    return (
        <div className="mb-6 flex flex-col gap-4 rounded-[28px] border border-white/70 bg-white/75 p-5 shadow-[0_18px_50px_-34px_rgb(15_23_42/0.28)] backdrop-blur-sm md:flex-row md:items-end md:justify-between md:p-6 dark:border-slate-700/60 dark:bg-slate-950/60">
    ...
    return (
        <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {items.map((item) => (
                <div
                    key={item.label}
                    className="rounded-[24px] border border-white/70 bg-white/82 p-4 shadow-[0_16px_40px_-30px_rgb(15_23_42/0.32)] backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-950/60"
                >
...
export function EntitySectionCard({ children, className = "" }: EntitySectionCardProps) {
    return (
        <section
            className={`rounded-[28px] border border-white/70 bg-white/82 p-5 shadow-[0_18px_50px_-34px_rgb(15_23_42/0.28)] backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-950/60 md:p-6 ${className}`.trim()}
        >
            {children}
        </section>
    );
